...
Section "Servidor Piccolo" SecServer
  ...
  CreateShortCut "$SMPROGRAMS\Piccolo TPV Server\Crear diagnóstico de soporte.lnk" "$SYSDIR\WindowsPowerShell\v1.0\powershell.exe" '-NoProfile -ExecutionPolicy Bypass -File "$INSTDIR\diagnostics\Collect-PiccoloDiagnostics.ps1" -AppRoot "$INSTDIR"'
