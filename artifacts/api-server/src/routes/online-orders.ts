...
// ── PUBLIC: POST /public/orders/online ────────────────────────────────────────

router.post("/public/orders/online", async (req, res): Promise<void> => {
  const {
    deliveryType,           // "takeaway" | "delivery"
    channel = "qr",         // "qr" | "web" | "phone"
    items,                  // [{ productId, quantity, formatId?, notes?, modifierIds? }]
    clientName,
    clientPhone,
    scheduledAt,            // ISO string or "asap"
    deliveryAddress,        // { street, number, floor, postalCode, city, notes }
    couponCode,
    paymentMethod = "online", // "online" | "on_arrival" | "on_delivery"
    saveAddress = false,
    consentRgpd = false,
  } = req.body as {
    deliveryType: string;
    channel?: string;
    /** NOTE: modifierIds only — no priceDelta accepted from client; prices come from DB */
  ...
  // ── Validate input ─────────────────────────────────────────────────────────
  if (!clientName?.trim()) { res.status(400).json({ error: "El nombre es obligatorio." }); return; }
  if (!clientPhone?.trim()) { res.status(400).json({ error: "El teléfono es obligatorio." }); return; }
  if (!Array.isArray(items) || items.length === 0) { res.status(400).json({ error: "El carrito está vacío." }); return; }
  if (!["takeaway", "delivery"].includes(deliveryType)) {
    res.status(400).json({ error: "Tipo de pedido no válido." }); return;
  }
  // Validate each item has a valid positive integer quantity
  for (const item of items) {
    if (!item.productId || typeof item.productId !== "string") {
      res.status(400).json({ error: "productId inválido en uno de los artículos." }); return;
    }
    const qty = Number(item.quantity);
    if (!Number.isInteger(qty) || qty < 1 || qty > 99) {
      res.status(400).json({ error: `Cantidad no válida para el producto ${item.productId}. Debe ser un entero entre 1 y 99.` }); return;
    }
  }

  // ── Load and validate service config ──────────────────────────────────────
  const [cfg] = await db.select().from(onlineOrdersConfigTable).limit(1);
  if (!cfg) { res.status(503).json({ error: "El servicio de pedidos online no está configurado." }); return; }

  if (cfg.paused) {
    res.status(503).json({ error: `El servicio está temporalmente pausado. ${cfg.pauseReason}` }); return;
  }
  if (deliveryType === "takeaway" && !cfg.takeawayEnabled) {
    res.status(400).json({ error: "El servicio de recogida no está disponible ahora mismo." }); return;
  }
  if (deliveryType === "delivery" && !cfg.deliveryEnabled) {
    res.status(400).json({ error: "El servicio de reparto no está disponible ahora mismo." }); return;
  }

  // Schedule check — only if no specific time was requested (asap = now)
  const isAsap = !scheduledAt || scheduledAt === "asap";
  const requestedAt = isAsap ? new Date() : new Date(scheduledAt);

  if (isAsap && !isWithinSchedule(cfg.schedule as any)) {
    res.status(400).json({ error: "El servicio no está disponible en este horario." }); return;
  }

  // ── Validate delivery zone ─────────────────────────────────────────────────
  let matchedZone: typeof deliveryZonesTable.$inferSelect | null = null;
  let effectiveDeliveryFee = parseFloat(cfg.deliveryFee);
  ...
  if (deliveryType === "delivery") {
    ...
    if (!matchedZone) {
      res.status(400).json({ error: "La dirección no está dentro de las zonas de reparto disponibles." }); return;
    }

    effectiveDeliveryFee = parseFloat(matchedZone.deliveryFee);
  }
  ...
  let subtotalLines: { lineTotal: number; taxRate: number }[] = [];
  const resolvedItems: Array<{
    ...
    /** Server-resolved modifiers — prices come from DB only */
  ...
  for (const item of items) {
    ...
    if (!product) {
      res.status(404).json({ error: `Producto no encontrado: ${item.productId}` }); return;
    }
    if (!product.active || !product.qrVisible) {
      res.status(400).json({ error: `El producto "${product.name}" no está disponible.` }); return;
    }
    if (product.outOfStock) {
      res.status(400).json({ error: `El producto "${product.name}" está agotado.` }); return;
    }

    let unitPrice = parseFloat(product.price);
    let taxRate = product.taxRate ?? 10;
    ...
    // ── Server-authoritative modifier pricing ─────────────────────────────────
    // Client may only send modifier UUIDs; prices are ALWAYS resolved from DB.
    // Any client-supplied priceDelta field is silently ignored.
    ...
    if (item.modifierIds?.length) {
      ...
      if (modIds.length) {
        // Load modifiers from DB, verifying they belong to this product
        const dbModifiers = await db
          .select({ id: modifiersTable.id, name: modifiersTable.name, priceDelta: modifiersTable.priceDelta, groupId: modifiersTable.groupId })
          .from(modifiersTable)
          .where(and(inArray(modifiersTable.id, modIds), eq(modifiersTable.active, true)));

        // Verify each modifier's group is actually linked to this product
        const groupIds = [...new Set(dbModifiers.map(m => m.groupId))];
        const linkedGroups = groupIds.length
          ? await db.select({ modifierGroupId: productModifierGroupsTable.modifierGroupId })
              .from(productModifierGroupsTable)
              .where(and(
                eq(productModifierGroupsTable.productId, item.productId),
                inArray(productModifierGroupsTable.modifierGroupId, groupIds),
              ))
          : [];
        const linkedGroupIds = new Set(linkedGroups.map(g => g.modifierGroupId));

        for (const m of dbModifiers) {
          if (!linkedGroupIds.has(m.groupId)) {
            res.status(400).json({ error: `El modificador "${m.name}" no pertenece al producto seleccionado.` }); return;
          }
  ...
  const { subtotal, taxTotal, total } = calcMultiRateBreakdown(subtotalLines, 0);
  const totalNum = parseFloat(total);
  const orderTotal = deliveryType === "delivery"
    ? (totalNum + effectiveDeliveryFee).toFixed(2)
    : total;

  // ── Minimum order checks ──────────────────────────────────────────────────
  const minOrder = deliveryType === "delivery"
    ? parseFloat(matchedZone?.minOrder ?? cfg.minOrderDelivery)
    : parseFloat(cfg.minOrder);

  if (totalNum < minOrder && minOrder > 0) {
    res.status(400).json({
      error: `El importe mínimo para ${deliveryType === "delivery" ? "reparto" : "recogida"} es ${minOrder.toFixed(2)} €. Tu pedido es de ${totalNum.toFixed(2)} €.`,
    }); return;
  }

  // Free delivery check
  const freeFrom = cfg.freeDeliveryFrom ? parseFloat(cfg.freeDeliveryFrom) : null;
  if (deliveryType === "delivery" && freeFrom && totalNum >= freeFrom) {
  ...
  try {
    const [existingClient] = await db.select()
      ...
      .where(eq(crmClientsTable.telefono, clientPhone.trim()))
      .limit(1);
    ...
    if (existingClient) {
      ...
      const [newClient] = await db.insert(crmClientsTable).values({
        ...
        rgpdConsentimiento: consentRgpd,
        rgpdFecha: consentRgpd ? new Date() : null,
      }).returning();
      crmClientId = newClient.id;
    }
  } catch { /* CRM lookup must not block order creation */ }

  // ── Calculate estimated ready time ────────────────────────────────────────
  const prepMinutes = cfg.prepTimeMinutes;
  const deliveryMinutes = deliveryType === "delivery"
    ? (matchedZone?.estimatedMinutes ?? 45)
    : 0;

  const estimatedReadyAt = new Date(
    (isAsap ? new Date() : requestedAt).getTime() + (prepMinutes + deliveryMinutes) * 60 * 1000,
  );
  ...
  const [order] = await db.insert(ordersTable).values({
    ...
    orderType: deliveryType,     // backward compat
    status: paymentMethod === "online" ? "pending_payment" : "pending_confirm",
    ...
    estimatedReadyAt,
    scheduledAt: isAsap ? null : requestedAt,
  ...
  // ── Audit + notification ───────────────────────────────────────────────────
  await auditOnlineOrder(order.id, "order_created", null, "cliente", {
    channel, deliveryType, itemCount: items.length, total: orderTotal, paymentMethod,
  });

  await simulateNotification(order.id, "order_received", clientPhone, {
    orderNumber,
    message: `Hemos recibido tu pedido ${orderNumber}. Te avisaremos en cuanto lo confirmemos.`,
  });

  emitOnlineOrdersRefresh({ orderId: order.id, orderNumber });
