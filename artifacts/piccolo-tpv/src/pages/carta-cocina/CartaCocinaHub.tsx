 ...
 * Agrupa: QR Menú (carta pública, categorías, productos, branding)
 *          y Food Cost (ingredientes, escandallos, compras, stock).
 */
        ...
        <div className="space-y-7">
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-[10px] px-2 py-0.5 rounded-full font-bold" style={{ background: FC_BG, color: FC_ACCENT }}>Costes y stock</span>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {FC_ITEMS.map(item => <HubCard key={item.href + item.title} item={item} />)}
            </div>
          </div>
        </div>
      </main>
