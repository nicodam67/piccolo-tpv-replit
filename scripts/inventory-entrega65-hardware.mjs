...
const capabilities = [
  ...
  item("print-worker-restart", "lib/print-worker.ts", "Resumes pending/retrying and expired sending leases with visible resend mark", "worker polling", "validated-software", "PostgreSQL integration covers interrupted lease recovery", "PostgreSQL", "print-worker.integration.test.ts", pendingPhysical("printer/service", "restart during sending"), "high", "certify-physically"),
  ...
  item("print-tcp-schema", "schema/printers.ts", "Stores TCP mode, address, port, codepage, cut/drawer and timeouts", "TCP/IP ESC/POS", "implemented-unverified", "admin CRUD and TCP software transport exist", "LAN printer", "tcp-escpos.test.ts", pendingPhysical("network printer", "TCP transport"), "critical", "certify-physically"),
  ...
  item("escpos-payload", "lib/print-connector/escpos-payload.ts", "Builds common ESC/POS init/codepage/text/feed/cut/drawer bytes", "ESC/POS common subset", "implemented-unverified", "CP858 payload and TCP tests pass; model-specific behavior unverified", "Real connector/printer", "escpos-payload.test.ts", pendingPhysical("ESC/POS printer", "charset, cut and drawer"), "critical", "certify-physically"),
  item("paper-cut", "lib/print-connector/escpos-payload.ts + admin print test", "Optional GS V cut command plus physical checklist", "ESC/POS common subset", "implemented-unverified", "byte command test passes; model compatibility unverified", "Compatible printer", "escpos-payload.test.ts", pendingPhysical("printer", "automatic cut"), "high", "certify-physically"),
  item("cash-drawer-kick", "lib/print-connector/escpos-payload.ts", "Optional explicit ESC/POS drawer pulse", "ESC/POS common subset", "implemented-unverified", "byte payload test passes; wiring/model unverified", "Drawer connected to compatible printer", "escpos-payload.test.ts", pendingPhysical("cash drawer", "open pulse"), "critical", "certify-physically"),
  item("production-print-connector", "lib/print-connector", "Per-printer simulator or TCP ESC/POS selection; simulator rejected in production", "TCP ESC/POS", "implemented-unverified", "real local TCP socket and fail-closed tests pass", "Physical LAN printer", "tcp-escpos.test.ts + print-worker.integration.test.ts", pendingPhysical("printer", "all production print cases"), "critical", "certify-physically"),
  ...
  item("kds-fixed-zones", "production_departments + KDS routes/UI", "Departments and navigation load dynamically from PostgreSQL", "internal registry", "validated-software", "CRUD and dynamic KDS integration tests pass", "Owner department design", "production-departments tests", pendingPhysical("KDS fleet", "final department list"), "high", "certify-physically"),
  ...
  item("department-mode-per-zone", "production_departments", "Independent none/KDS/printer/both plus ordered printers; global mode remains ceiling", "configuration", "validated-software", "channel matrix, CRUD and order regressions pass", "Department design", "production-departments.test.ts", pendingPhysical("KDS/printer departments", "all four modes"), "critical", "certify-physically"),
